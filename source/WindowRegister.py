...
class WindowRegister(QtWidgets.QDialog, register.Ui_register_form):
    def __init__(self, parent=None):
        ...
        self.login_validator = QtGui.QRegularExpressionValidator(QtCore.QRegularExpression("[A-Za-z0-9_]{3,15}"),
                                                                 self.login_line)
        self.login_line.setValidator(self.login_validator)
        # Для пароля - те же правила, но без нижнего подчёркивания
        self.password_validator = QtGui.QRegularExpressionValidator(QtCore.QRegularExpression("[A-Za-z0-9]{8,20}"),
                                                                    self.password_line)
        self.password_line.setValidator(self.password_validator)
        # Для имени - латинские буквы, и кириллица
        self.name_validator = QtGui.QRegularExpressionValidator(QtCore.QRegularExpression(r'\w+=\p{L}+\s*'),
                                                                self.name_line)
        self.name_line.setValidator(self.name_validator)
        # Для должности всё то же самое
        self.post_validator = QtGui.QRegularExpressionValidator(QtCore.QRegularExpression(r'\w+=\p{L}+\s*'),
                                                                self.post_line)
        self.post_line.setValidator(self.post_validator)
        # И для компании - тоже
        self.company_validator = QtGui.QRegularExpressionValidator(QtCore.QRegularExpression(r'\w+=\p{L}+\s*'),
                                                                   self.company_line)
